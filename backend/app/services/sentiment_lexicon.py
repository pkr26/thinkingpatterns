"""The VADER base lexicon (2026-09-17) — machine-merged, human-gated.

Source: vaderSentiment's vader_lexicon.txt (MIT license, Hutto & Gilbert
2014), 7,520 graded entries; the engine previously carried a hand-curated
~370-word lexicon. Merge rules (applied at generation, pinned by the
sentiment tests):

  * emoticon/symbol entries dropped (the tokenizer's WORD_RE = [a-z']+
    would never match them);
  * the curation-documented context-dependent words ("kind", "fed",
    "present") stay REMOVED — the journal-register curation wins over
    dictionary valence;
  * valences kept on VADER's native [-4, 4] scale (the same convention
    the curated lexicon already uses); zero-valence entries dropped;
  * the curated SENTIMENT_LEXICON in brain.py OVERRIDES these values
    word-for-word at import (brain.py merges: {**VADER_BASE, **CURATED}).

Determinism: a plain dict literal, imported like any module — no runtime
file parsing, no ordering dependence.

Emoji valences: a small journal-relevant subset (the full VADER emoji
lexicon is available for a follow-up); extracted by their own pass
alongside WORD_RE so they score mood without entering the phrase/theme
pipelines.
"""

from __future__ import annotations

VADER_BASE: dict[str, float] = {
    'aas': 2.5,
    'aayf': 2.7,
    'abandon': -1.9,
    'abandoned': -2.0,
...
#: Journal-plausible emoji valences ([-4, 4], VADER's scale).
EMOJI_VALENCES: dict[str, float] = {
    "😀": 2.8, "😃": 2.8, "😄": 3.0, "😁": 2.6, "😊": 2.4, "🙂": 1.6,
    "😍": 3.1, "🥰": 3.1, "😆": 2.4, "😅": 1.1,
    "😢": -2.6, "😭": -3.0, "😞": -2.2, "😔": -2.3, "🙁": -1.7, "☹️": -2.2,
    "😩": -2.6, "😫": -2.7, "😰": -2.8, "😨": -2.8,
    "😠": -2.8, "😡": -3.1, "💔": -2.9, "😴": 0.4, "🥱": -0.5,
    "✨": 1.9, "🌈": 2.0, "☀️": 1.7, "🌧️": -1.0, "⛈️": -1.3,
    "💪": 2.0, "🙏": 1.3, "❤️": 3.0, "💜": 2.6, "🫂": 1.6,
}
