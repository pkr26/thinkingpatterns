...
still does `create_all`, but that only ever helps a *fresh* database — it
silently ignores every later change to `app/models.py`. Any column/table
change MUST ship as a revision here.

All commands run from `backend/`. The database URL is read from the same
place the app reads it (`app.config.Settings.from_env` → `MINDPATTERN_DB_URL`),
so there is exactly one configuration path — no `sqlalchemy.url` in
`alembic.ini` to drift.
...
Run this at deploy time, before starting the new app version. SQLite dev
