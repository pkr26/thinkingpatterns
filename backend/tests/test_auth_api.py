...
async def test_login_wrong_password_rejected(client):
    ...
    response = await client.post("/api/auth/login", json={
        "username": "carol", "verifier": imposter.auth_key_b64,
    })
...
async def test_login_unknown_user_rejected(client):
    response = await client.post("/api/auth/login", json={
        "username": "ghost", "verifier": base64.b64encode(b"\x00" * 32).decode(),
    })
    assert response.status_code == 401
...
async def test_register_rejects_bad_salt_and_b64(client):
    for salt, verifier in [
        (base64.b64encode(b"tiny").decode(), base64.b64encode(b"1" * 32).decode()),  # salt < 8 bytes
        ("!!!not-b64!!!", base64.b64encode(b"1" * 32).decode()),
        (base64.b64encode(b"0" * 16).decode(), "%%%not-b64%%%"),
    ]:
        response = await client.post("/api/auth/register", json={
            "username": "erin", "salt": salt, "verifier": verifier,
        })
        assert response.status_code == 422
...
async def test_logout_revokes_every_token(client):
    ...
    ok = await client.get(
        "/api/entries", headers={"Authorization": f"Bearer {relogged['token']}"}
    )
    assert ok.status_code == 200


async def test_protected_routes_require_token(client):
    for method, path in [("get", "/api/entries"), ("post", "/api/insights/recompute"),
                         ("get", "/api/insights"), ("get", "/api/questions/today"),
