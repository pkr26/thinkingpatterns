...
| `mutation_campaign_2026-09-22_frontend/` | — | frontend round (portal + mobile): fresh full-scope Stryker over BOTH apps on the current tree — portal all of `src` for the first time with the working command runner (3,549 mutants), mobile's first full run since 2026-09-15 (15,990 mutants); found and fixed a source-text pin that would have broken the mobile gate's dry run (see its `REPORT.md`) |
