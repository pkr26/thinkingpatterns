 ...
 * the RN toolchain at build time), so the Apple Health mirror is a SEAM
 * now and goes live the moment the native module links — exactly the
 * notifee/biometrics pattern in nativeFeatures.ts: capability probing via
 * require/dynamic-import, a missing module degrades to "unavailable",
 * never a crash, and every write path returns false rather than throwing.
 *
...
/** Synchronous capability probe for Settings. A module that is present
 *  but lacks the State of Mind write path gets its own honest reason —
 *  "not linked" and "too old to write State of Mind" are different
 *  facts for the person reading the reason line. */
