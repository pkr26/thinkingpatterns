/**
 * Crypto/Buffer polyfill installation — extracted from index.js so it can
 * be required (and tested) on its own.
 *
 ...
 *
 * require() (not ESM import) is deliberate throughout: every import in an
 * ES module would be hoisted and evaluated BEFORE this module's body, so
 * App's whole module graph would still load before install() executed.
 * Sequential requires in index.js keep the install strictly first.
 */
