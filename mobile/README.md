...
This repo ships JS/TS source only; `ios/` and `android/` are generated by
the React Native toolchain and are **not committed**. `npm run
verify:native-release` therefore fails closed until they exist. Work
through this guide in order; every numbered item maps to a check the
preflight tool runs (or a deliberate, documented decision).
